...
local ChargeDay = class(LuaController)

function ChargeDay:createDocument()
    self._factory:setZipFilePath(Config.COCOS_ZIP_DIR.."ChargeDay.cocos.zip")
    return self._factory:createDocument("ChargeDay.cocos")
end

--@@@@[[[[
function ChargeDay:onInitXML()
    local set = self._set
   ...
   self._cnt_tail = set:getLabelNode("cnt_tail")
   self._title1 = set:getLinearLayoutNode("title1")
   self._title1_V = set:getLabelNode("title1_V")
   self._title1_tail = set:getLabelNode("title1_tail")
   ...
   self._title2_tail = set:getLabelNode("title2_tail")
...
--------------------------------override functions----------------------
function ChargeDay:onInit( userData, netData )
	
end

function ChargeDay:onBack( userData, netData )
...
updateLayer = function ( self,view )
  ...
  local info = appData.getActivityInfo().getDataByType(4)
  print('info:')
  print(info)
  ...
  if lastseconds < 0 then
    ...
    print('活动已过期！')
    self:refreshActivityInfo(view,4,function ( data )
      if data then
        updateLayer(self,view)
      else
        checkRedPoint(self)
  ...
  view['title2_tail']:setString(string.format(Res.locString('Activity$ChargeAccDay'),tonumber(netData.Day)))
...
updateList = function ( self,view,info )
  ...
  local foucs = 0
  if info.Data then
    for i,v in ipairs(info.Data.Rewards) do
  ...
  view['title1_V']:setString(tostring(info.Data.Coin))
end
...
refreshDayCell = function (self, set, v,i,view,hadget)
  ...
  set['cnt_V']:setString(tostring(i))  
  set['btnGet']:setEnabled(netData.Day >= i and not hadget)
...
checkRedPoint = function ( self )
  ...
  if info and info.Data then
    for i,v in ipairs(info.Data.Rewards) do
        local hadget = hadGet(i)
        if (netData.Day >= i and not hadget) then
          return
        end
    end
  end
  ...
  appData.getActivityInfo().activityEnd(4)
  self:onActivityFinish(require 'ActivityType'.ChargeDay)
  -- GleeCore:popController(nil,Res.getTransitionFade())
end

--net
getReward = function ( self,Day,view )
  self:send(netModel.getDChReward(Day),function ( data )
    if data.D then
      if netData.Gots  == nil then
        netData.Gots = {}
      end
      table.insert(netData.Gots,Day)
