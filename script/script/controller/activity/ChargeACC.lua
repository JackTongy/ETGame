...
local ChargeACC = class(LuaController)

function ChargeACC:createDocument()
    self._factory:setZipFilePath(Config.COCOS_ZIP_DIR.."ChargeACC.cocos.zip")
    return self._factory:createDocument("ChargeACC.cocos")
end

--@@@@[[[[
function ChargeACC:onInitXML()
    local set = self._set
...
--------------------------------override functions----------------------
function ChargeACC:onInit( userData, netData )
	
end

function ChargeACC:onBack( userData, netData )
...
updateLayer = function ( self,view )
  local info = appData.getActivityInfo().getDataByType(5)
  ...
  if lastseconds < 0 then
    ...
    print('活动已过期！')
    self:refreshActivityInfo(view,5,function ( data )
      if data then
        updateLayer(self,view)
      else
        checkRedPoint(self)
...
checkRedPoint = function ( self )
  info = appData.getActivityInfo().getDataByType(5)
  ...
  appData.getActivityInfo().activityEnd(5)
  self:onActivityFinish(require 'ActivityType'.ChargeACC)
...
getReward = function ( self,N,view )
  self:send(netModel.getTChReward(N),function ( data )
    if data.D then
