...
local update = function ( self,view,data,dbBoss)
  ...
  eventCenter.addEventFunc('OnBattleCompleted',function ( data )
    GleeCore:showLayer('DBossBattle')
  end,'BossBattleObs')
...
updateLayer = function ( self,view )
	require "LangAdapter".selectLang(nil,nil,nil,nil,nil,function ( ... )
		view["wait_Tip2"]:setVisible(false)
		view["wait_#lineartime"]:setPosition(ccp(5.7143555,42.57141))
		view["wait_lineartime_#label"]:setFontSize(32)
		view["wait_lineartime_time"]:setFontSize(32)
	end, function ( ... )
		view["wait_Tip2"]:setVisible(false)
		view["wait_#lineartime"]:setPosition(ccp(5.7143555,42.57141))
		view["wait_lineartime_#label"]:setFontSize(32)
  ...
  local sc = TimeManager.getCurrentSeverTime()/1000 +(os.time() - os.time(os.date("!*t")))
  view['severtime']:setHourMinuteSecond(0,0,sc)
  ...
  require 'LangAdapter'.fontSize(view['battleing_btnStart_title'],nil,nil,nil,nil,22)
  require 'LangAdapter'.fontSize(view['title1'],nil,nil,nil,nil,20)
  require 'LangAdapter'.LabelNodeAutoShrink(view['bossnamelv_V'],160)
end
...
updateWait = function ( self,view )
  ...
  if offset > 0 then
    ...
    lineartime:setHourMinuteSecond(0,0,offset+2)
  ...
  view['timeicon']:setVisible(false)
...
updateBattling = function ( self,view )
  ...
  if lastsec > 0 then
    ...
    time:setHourMinuteSecond(0,0,lastsec+2)
...
getBossData = function ( ... )
  ...
  local Boss = {}
  Boss.Pet = appData.getPetInfo().getPetInfoByPetId(curdbBoss.PetId,24)
  ...
  if harms and type(harms) == 'table' then
    ...
    for i,v in ipairs(harms) do
      ...
      self:runWithDelay(function ( ... )
        local dmgset = self:createLuaSet('@dmg')
        local hpnode = dmgset[1]
        local namenode = dmgset['pname']
        
        local hpshow = self:createLuaSet('hpShow')[1]
        local quake = self:createLuaSet('quake')[1]
        hpnode:setString(string.format('-%s',tostring(v.Harm)))
        namenode:setString(tostring(v.Name))
        local color1,color2 = getDmgColor(v.Harm)
        hpnode:setFontFillColor(color1,true)
        namenode:setFontFillColor(color2,true)
...
getDmgColor = function ( harm )
  ...
  if v > 0 and v <= 20000 then
    return ccc4f(0.988235,0.266667,0.121569,1.0),ccc4f(0.988235,0.296667,0.141569,1.0)
  elseif v > 20000 and v <= 40000 then
    return ccc4f(0.992157,0.584314,0.152941,1.0),ccc4f(0.992157,0.654314,0.172941,1.0)
  elseif v > 40000 then
    return ccc4f(0.984314,0.921569,0.203922,1.0),ccc4f(0.984314,0.971569,0.223922,1.0)
  end
  return ccc4f(0.988235,0.266667,0.121569,1.0),ccc4f(0.988235,0.296667,0.141569,1.0)
...
getOpenTimeOffset = function ( ... )
  ...
  -- print('--------getopen')
  -- print(offset)
  -- print(netData.BossAtkGet.BossAtk.OpenTime)
  return -offset
end

local TLBoss = class(TabLayer)

function TLBoss:createDocument()
    self._factory:setZipFilePath(Config.COCOS_ZIP_DIR.."TLBoss.cocos.zip")
    return self._factory:createDocument("TLBoss.cocos")
end

--@@@@[[[[
function TLBoss:onInitXML()
    local set = self._set
   self._title1 = set:getLabelNode("title1")
   self._timeicon = set:getElfNode("timeicon")
   ...
   self._battleing_btnStart_title = set:getLabelNode("battleing_btnStart_title")
   ...
   self._pname = set:getLabelNode("pname")
   self._severtime = set:getTimeNode("severtime")
...
function TLBoss:onInit( userData, netData )
	update(self,self._viewSet,self._parent._netData,self._parent._curdbBoss)
end

function TLBoss:onBack( userData, netData )
	
end

function TLBoss:onRelease( ... )
  releaseTick()
...
--------------------------------class end-------------------------------
require 'framework.basic.MetaHelper'.classDefinitionEnd(TLBoss, "TLBoss")
