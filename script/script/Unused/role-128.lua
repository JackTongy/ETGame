...
local role-128 = class(LuaController)

function role-128:createDocument()
    self._factory:setZipFilePath(config.COCOS_ZIP_DIR.."role-128.cocos.zip")
    return self._factory:createDocument("role-128.cocos")
end

--@@@@[[[[  created at Thu May 29 15:12:22 CST 2014 By null
function role-128:onInitXML()
    local set = self._set
...
--------------------------------override functions----------------------
function role-128:onInit( userData, netData )
	
end

function role-128:onBack( userData, netData )
...
--------------------------------class end-------------------------------
require 'framework.basic.MetaHelper'.classDefinitionEnd(role-128, "role-128")


--------------------------------register--------------------------------
GleeCore:registerLuaController("role-128", role-128)
